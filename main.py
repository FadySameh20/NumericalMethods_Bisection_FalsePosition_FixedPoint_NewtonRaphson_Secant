...
def plot(equ_str, prec):
    ...
    f = lambdify('x', expr)
    # list of squares
    # y = [f(i) for i in range(int(lower.get()), int(higher.get()))]
    # y = [f(i) for i in numpy.linspace(int(lower.get()), int(higher.get()), 1000).tolist()]
    y = [f(i) for i in numpy.linspace(int(lower.get()), int(higher.get()), 100).tolist()]

    # adding the subplot
    # plot1 = fig.add_subplot(111)
    if iters.getvar() == 1:
        plot1.axvline(x=float(lower.get()), color='green')
        plot1.axvline(x=float(higher.get()), color='purple')
        plot1.axvline(x=(float(lower.get()) + float(higher.get())) / 2, color='red')
    ...
    # plotting the graph
    plot1.plot([i for i in numpy.linspace(int(lower.get()), int(higher.get()), 100).tolist()], y)
...
def bisection(l, u, f, prec, iter, old_mid=None):
    ...
    iters.setvar(value=int(iters.getvar()) + 1)
    if iter == 1 or numpy.isclose(fmid, 0):
        return mid, ea, iter
...
def fixedPoint(xr, g, prec, iter):
    ...
    if g(xr) == 0:
        ea = "Exact"
    treev.insert("", 'end', values=(iters.getvar(), xr_old, g(xr_old), "", "", xr, g(xr_old), ea))
    iters.setvar(value=int(iters.getvar()) + 1)
    if ea > prec:
        if iter == 1 or numpy.isclose(g(xr), 0):
            return xr, ea, iter
        iter = iter - 1
...
def newton_raphson(x_i_1, f, f_prime, prec, iter):
    ...
    iters.setvar(value=int(iters.getvar()) + 1)
    if iter == 1 or (not ea and ea <= prec) or numpy.isclose(f(x_i), 0):
        return x_i, ea, iter
    else:
...
def secant_method(x_i_1, x_i, f, prec, iter):
    ...
    iters.setvar(value=int(iters.getvar()) + 1)
    if iter == 1 or (not ea and ea<= prec) or numpy.isclose(f(x_new), 0):
        return x_new, ea, iter
    else:
    ...
    try:
        ...
        if scanner == 0 or scanner == 1:
            ...
            if fl * fu < 0:
                if scanner == 0:
                    enable(plot_button)
                    root, ea, iterRes = bisection(float(lower.get()), float(higher.get()), f, prec, iter)
                elif scanner == 1:
                    root, ea, iterRes = falseposition(float(lower.get()), float(higher.get()), f, prec, iter)
            elif numpy.isclose(fl, 0):
                root = float(lower.get())
                ea = None
            elif numpy.isclose(fu, 0):
            ...
            if abs(f_prime(float(lower.get()))) < 1:
                root, ea, iterRes = fixedPoint(float(lower.get()), lambdify('x', expr), prec, iter)
            else:
                root = None
                isFixedDiverging = 1
        elif scanner == 3:
            root, ea, iterRes = newton_raphson(float(lower.get()), lambdify('x', sympify(equ_str)),
                                               lambdify('x', diff(sympify(equ_str))),
                                               prec, iter)
        elif scanner == 4:
            root, ea, iterRes = secant_method(float(lower.get()), float(higher.get()), lambdify('x', expr), prec, iter)
        end = time.time()
        if root is None:
            ...
            root = "Root is " + str(root) + ", Relative error is " + str(ea) + ", Number of iterations are " + str(
                int(iters.get()) - iterRes + 1) + ", Execution Time is " + str(end - start)
        resetCanvasAndToolbar()
        iters.setvar(value=1)
